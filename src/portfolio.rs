...
impl Portfolio {
	...
	pub fn new(initial_cash: f64) -> PyResult<Self> {
		if initial_cash <= 0.0 {
			Err(pyo3::exceptions::PyValueError::new_err("Initial cash must be positive"))
		} else {
			Ok(Portfolio {
				initial_cash,
				cash: initial_cash,
	...
	pub fn can_buy(&self, symbol: &str, quantity: i32, price: f64) -> bool {
		quantity > 0 && price > 0.0 && self.cash >= (quantity as f64 * price)
	}
	...
	pub fn execute_trade(&mut self, timestamp: String, symbol: String, action: String, quantity: i32, price: f64) -> bool {
		let trade = Trade::new(timestamp, symbol.clone(), action.clone(), quantity, price);
		if action == "BUY" && self.can_buy(&symbol, quantity, price) {
			self.cash -= trade.value;
			*self.positions.entry(symbol.clone()).or_insert(0) += quantity;
